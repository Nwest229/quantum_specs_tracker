...
from .models import BackendRecord, F
...
COL = {
    ...
    "tech_date": 30,
    "tech_source": 31,
    "mid_circuit_measurement": 32,
    "conditional_logic": 33,
    "parallel_2q": 34,
    "qubit_reuse": 35,
    "hybrid_execution": 36,
    "uptime": 37,
...
def de_num(text: str) -> float | None:
...
def de_percent(text: str) -> float | None:
...
def de_money(text: str) -> float | None:
...
def _source_kind(url: str) -> str:
    ...
    if "aws.amazon" in u or "azure" in u or "learn.microsoft" in u:
        return "cloud-api"
    if "wikipedia" in u or "quantumcomputingreport" in u or "venturebeat" in u or "linkedin" in u:
        return "third-party"
    return "vendor"


def _backend_name(row: list[str]) -> str:
...
def records_from_csv(path: str | Path) -> Iterable[BackendRecord]:
    path = Path(path)
    if not path.exists():
        return
...
def _record(row: list[str], vendor: str) -> BackendRecord:
    ...
    if aq is not None:
        rec.black_box = F(
            int(aq) if aq == int(aq) else aq,
            tsrc if _known(tsrc) else "",
            tdate,
            "vendor-spec",
            tkind,
        )
    for name in ("argmax", "vendor_metric"):
        ...
        if _known(raw):
            setattr(rec, name, F(raw, tsrc if _known(tsrc) else "", tdate, "vendor-spec", tkind))
    ...
    def _to_rounded_int(x: str) -> int | None:
        n = de_num(x)
        return round(n) if n is not None else None

    put("qpu_topology.qubits", row[COL["qubits"]], _to_int, "vendor-spec")
    if _known(row[COL["topo_type"]]):
        rec.set(
            "qpu_topology.type",
            F(
                _clean(row[COL["topo_type"]]),
                tsrc if _known(tsrc) else "",
                tdate,
                "vendor-spec",
                tkind,
    ...
    put("qpu_topology.edges", row[COL["edges"]], _to_rounded_int, "vendor-spec")
    ...
    for k in (
        ...
        if _known(raw):
            rec.set(
                f"features.{k}", F(raw, tsrc if _known(tsrc) else "", tdate, "vendor-spec", tkind)
            )

    # pricing: (ct) cols are US cents -> USD dollars ; € cols -> EUR
    def _cents_to_dollars(x: str) -> float | None:
        n = de_num(x)
